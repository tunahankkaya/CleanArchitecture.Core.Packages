    ...
    where TRequest : IRequest<TResponse>, ICachableRequest
{
    ...
    private readonly IDistributedCache _cache; //microsoft tarafından sunulan standart cache interface i 
    ...
    private async Task<TResponse?> getresponseAndAddToCache(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        TimeSpan slidinExpiration = request.SlidingExpiration ?? TimeSpan.FromDays(_cacheSettings.SlidingExpiration);
        DistributedCacheEntryOptions cacheOptions = new() { SlidingExpiration = slidinExpiration };
